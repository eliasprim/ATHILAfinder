...
pidtab1=read.table(input4, sep="\t", header = T)
...
pidtab2=read.table(input5, sep="\t", header = T)
...
pidtab3=read.table(input6, sep="\t", header = T)
...
pidtab4=read.table(input7, sep="\t", header = T)
...
allflsolotsd=allflsolotsd %>%
  mutate(species=as.character(input10))


write.table(allflsolotsd, input11, sep = "\t", row.names = F, quote= F)
...
hmm=read.table(input12, sep="\t", header=T)
...
write.table(allflhmm, input13, sep = "\t", row.names = F, quote= F)
